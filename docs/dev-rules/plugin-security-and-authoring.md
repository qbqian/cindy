...
- 市场列表与详情的发布身份校验保持插件 ID、ghostId、scope、组织归属、Release ID、版本、
  包 SHA-256 和大小一致；`defaultInstall` 是动态安装资格，不属于发布身份。部门查询失败或
  调岗导致该字段变化时，详情与手动安装仍可继续；首次后台自动安装必须同时满足列表与
  最新详情的 `defaultInstall=true`，详情返回 false 时跳过，用户退订检查继续保留。
